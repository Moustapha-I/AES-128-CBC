import base64

# AES substitution box used in encryption and key expansion to perform byte substitution
s_box = (
    ,
)

# AES inverse S-box used in decryption to reverse the byte substitution step
inv_s_box = (
    ,
)

# AES round constants used during key expansion to generate round keys
r_con = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)

# inputs are taken as static variable from the code for simplicity (UI version takes the input from the user)
plaintext = "testing AES encrytion!"
key = "MySecret@#2025p["
IV = "76485930f3sjr7oe"

expanded_key = [] # contains 11 key rounds used in the encryption and decryption process

# add padding to plaintext so its length is a multiple of the AES block size (16 bytes)
def pad(plaintext):
    plaintextbytes = plaintext.encode()
    padding_len = 16 - (len(plaintextbytes) % 16)
    padding = bytes([padding_len] * padding_len)
    return plaintextbytes + padding

# remove padding from decrypted plaintext after AES decryption
def unpad(padded_plaintext):
    padding_len = padded_plaintext[-1]
    return padded_plaintext[:-padding_len]

# XOR two byte sequences
def xor_bytes(a, b):
    return bytes(i^j for i, j in zip(a, b))

# XORs the ciphertext or plaintext (16 bytes blocks) with the round key during encryption/decryption
def add_round_key(text, key):
    return xor_bytes(text, key)

# substitutes each byte in the state using the S-box during encryption
def sub_bytes(pbytes):
    for i in range(4):
        for j in range(4):
            val = pbytes[i][j][0]  
            pbytes[i][j] = bytes([s_box[val]])
    return pbytes

# substitutes each byte in the state using the inverse S-box during decryption (inverse of sub_bytes)
def inv_sub_bytes(pbytes):
    for i in range(4):
        for j in range(4):
            val = pbytes[i][j][0]
            pbytes[i][j] = bytes([inv_s_box[val]])
    return pbytes

# convert a flat byte array into a 4x4 matrix
def bytes_to_bytesmatrix(plainbytes):
    index = 0

    matrix = []
    for i in range(4):
        matrix.append([])
        for j in range (i, i+13, 4):
            matrix[i].append(bytes([plainbytes[j]]))
            index += 1
    
    return matrix

# shifts rows of the state left to provide diffusion during encryption
def shift_rows(bytes_matrix):
    bytes_matrix[1] = bytes_matrix[1][1:] + bytes_matrix[1][:1]
    bytes_matrix[2] = bytes_matrix[2][2:] + bytes_matrix[2][:2]
    bytes_matrix[3] = bytes_matrix[3][3:] + bytes_matrix[3][:3]
    return bytes_matrix

#  reverses the row shifts during decryption (shifts to the right)
def inv_shift_rows(bytes_matrix):
    bytes_matrix[1] = bytes_matrix[1][-1:] + bytes_matrix[1][:-1]
    bytes_matrix[2] = bytes_matrix[2][-2:] + bytes_matrix[2][:-2]
    bytes_matrix[3] = bytes_matrix[3][-3:] + bytes_matrix[3][:-3]
    return bytes_matrix

# multiply a byte by 2 in GF(2^8) for AES MixColumns transformation
def xtime(a):
    if a & 0x80:
        return ((a << 1) ^ 0x1B) & 0xFF
    else:
        return (a << 1) & 0xFF
    
def mul_by_2(x): return xtime(x)
def mul_by_3(x): return xtime(x) ^ x

def mul_by_9(x):  return xtime(xtime(xtime(x))) ^ x
def mul_by_11(x): return xtime(xtime(xtime(x)) ^ x) ^ x
def mul_by_13(x): return xtime(xtime(xtime(x) ^ x)) ^ x
def mul_by_14(x): return xtime(xtime(xtime(x) ^ x) ^ x)

# mixes one column of the state using GF multiplication during encryption
def mix_single_column(col):
    a = [col[i][0] for i in range(4)]
    b = [xtime(a[i]) for i in range(4)]
    c0 = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1]
    c1 = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2]
    c2 = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3]
    c3 = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0]
    return [bytes([c0]), bytes([c1]), bytes([c2]), bytes([c3])]

# reverses the column mixing during decryption
def inv_mix_single_column(col):
    a = [col[i][0] for i in range(4)]

    c0 = mul_by_14(a[0]) ^ mul_by_11(a[1]) ^ mul_by_13(a[2]) ^ mul_by_9(a[3])
    c1 = mul_by_9(a[0])  ^ mul_by_14(a[1]) ^ mul_by_11(a[2]) ^ mul_by_13(a[3])
    c2 = mul_by_13(a[0]) ^ mul_by_9(a[1])  ^ mul_by_14(a[2]) ^ mul_by_11(a[3])
    c3 = mul_by_11(a[0]) ^ mul_by_13(a[1]) ^ mul_by_9(a[2])  ^ mul_by_14(a[3])

    return [bytes([c0]), bytes([c1]), bytes([c2]), bytes([c3])]

# applies MixColumns to all state columns to provide diffusion during encryption
def mix_columns(bytes_matrix):
    for c in range(4):
        col = [bytes_matrix[r][c] for r in range(4)]
        mixed = mix_single_column(col)
        for r in range(4):
            bytes_matrix[r][c] = mixed[r]
    return bytes_matrix

# reverses MixColumns on all state columns during decryption
def inv_mix_columns(bytes_matrix):
    for c in range(4):
        col = [bytes_matrix[r][c] for r in range(4)]
        mixed = inv_mix_single_column(col)
        for r in range(4):
            bytes_matrix[r][c] = mixed[r]
    return bytes_matrix

# generates all round keys from the initial cipher key for encryption and decryption
def expand_key():
    key_matrix = [bytes(key[i:i+4]) for i in range(0, len(key), 4)]

    i = 1
    while len(key_matrix) < (11) * 4:
        word = list(key_matrix[-1])

        if len(key_matrix) % 4 == 0:
            word.append(word.pop(0))

            word = [s_box[x] for x in word]

            word[0] ^= r_con[i]
            i += 1

        word = xor_bytes(word, key_matrix[-4])
        key_matrix.append(word)

    return [b''.join(key_matrix[j:j+4]) for j in range(0, len(key_matrix), 4)]

# encrypts a single 16-byte block using AES with all rounds applied
def encrypt_block(plaintext):

    plaintext = add_round_key(plaintext, expanded_key[0])

    for i in range(1, 10):
        plaintext_matrix = bytes_to_bytesmatrix(plaintext)
        plaintext_matrix = sub_bytes(plaintext_matrix)
        plaintext_matrix = shift_rows(plaintext_matrix)
        plaintext_matrix = mix_columns(plaintext_matrix)
        plaintext = b''.join(plaintext_matrix[row][col] for col in range(4) for row in range(4))
        plaintext = add_round_key(plaintext, expanded_key[i])
    
    plaintext_matrix = bytes_to_bytesmatrix(plaintext)

    plaintext_matrix = sub_bytes(plaintext_matrix)
    plaintext_matrix = shift_rows(plaintext_matrix)
    plaintext = b''.join(plaintext_matrix[row][col] for col in range(4) for row in range(4))
    plaintext = add_round_key(plaintext, expanded_key[-1])

    return plaintext

# decrypts a single 16-byte block using AES with all rounds reversed
def decrypt_block(ciphertext):
    plaintext = add_round_key(ciphertext, expanded_key[-1])

    plaintext_matrix = bytes_to_bytesmatrix(plaintext)

    plaintext_matrix = inv_shift_rows(plaintext_matrix)
    plaintext_matrix = inv_sub_bytes(plaintext_matrix)
    plaintext = b''.join(plaintext_matrix[row][col] for col in range(4) for row in range(4))
    plaintext = add_round_key(plaintext, expanded_key[9])

    for i in range(8, -1, -1):
        plaintext_matrix = bytes_to_bytesmatrix(plaintext)
        plaintext_matrix = inv_mix_columns(plaintext_matrix)
        plaintext_matrix = inv_shift_rows(plaintext_matrix)
        plaintext_matrix = inv_sub_bytes(plaintext_matrix)
        plaintext = b''.join(plaintext_matrix[row][col] for col in range(4) for row in range(4))
        plaintext = add_round_key(plaintext, expanded_key[i])

    return plaintext

# encrypt plaintext using AES in CBC mode with padding and initialization vector (IV)
def encrypt(plaintext):
    plaintext = pad(plaintext)

    ciphertext = b""

    previous = IV
    for i in range(0, len(plaintext), 16):
        block = plaintext[i:i+16]
        ct_block = encrypt_block(xor_bytes(block, previous))
        ciphertext += ct_block
        previous = ct_block

    return base64.b64encode(ciphertext).decode()

# decrypt ciphertext using AES in CBC mode removing padding and using the IV
def decrypt(ciphertext):
    ciphertext = base64.b64decode(ciphertext.encode('ascii'))
    plaintext = b""
    previous = IV

    for i in range(0, len(ciphertext), 16):
        ct_block = ciphertext[i:i+16]
        decrypted_block = decrypt_block(ct_block)
        pt_block = xor_bytes(decrypted_block, previous)
        plaintext += pt_block
        previous = ct_block

    plaintext = unpad(plaintext)
    return plaintext.decode()

# plaintext to bytes
key = key.encode()

# plaintext to bytes
IV = IV.encode()

# store the round keys into global variable
expanded_key = expand_key()

# encryption process
ciphertext = encrypt(plaintext)

print("Encrypted:", ciphertext)

# decryption process
plaintext2 = decrypt(ciphertext)

print("Decrypted:", plaintext2)